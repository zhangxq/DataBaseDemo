...
abstract public class AsyncResponseHandler {
	...
	static class MsgHandler extends Handler {
		...
		private final WeakReference<AsyncResponseHandler> callback;

		MsgHandler(AsyncResponseHandler callback) {
			this.callback = new WeakReference<AsyncResponseHandler>(callback);
		}

		@Override
		public void handleMessage(Message msg) {
			AsyncResponseHandler asyncResponseHandler = callback.get();
			if (asyncResponseHandler != null) {
				asyncResponseHandler.handleMessage(msg);
			}
		}
	}
